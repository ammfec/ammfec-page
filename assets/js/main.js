(function ($) {
  ...
  $("#txt_app").hide();
  $("#txt_consultoria").hide();
  $("#txt_mantenimiento").hide();
  $("#txt_web").hide();
  ...
  $("#consultoria").click(function () {
    if (!bandera2) {
      $("#txt_consultoria").fadeIn();
      bandera2 = true;
    } else {
      $("#txt_consultoria").fadeOut();
      bandera2 = false;
    }
  });
  ...
  $("#mantenimiento").click(function () {
    if (!bandera3) {
      $("#txt_mantenimiento").fadeIn();
      bandera3 = true;
    } else {
      $("#txt_mantenimiento").fadeOut();
      bandera3 = false;
    }
  });
  ...
  $("#target").submit(function (event) {
   ...
   datosEmail.fullName=datosEmail.fullName.trim();
    let validEmail=/[a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*@[a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,5}/;
    let validNumber=datosEmail.tel.length;

    if (datosEmail.fullName.length<5) {
      ErrorMsj();
      return;  
    }else if(!validEmail.test(datosEmail.email)){
      ErrorMsj();
      return
    }else if(validNumber<8){
      ErrorMsj();
      return
    }else if (datosEmail.services.length==0) {
      ErrorMsj();
      return
    }

   let dateForm={
      "applicant":datosEmail.fullName, 
      "contact":'Email: '+datosEmail.email+' Telefono: '+datosEmail.tel, 
      "description":datosEmail.services.toString()
    }
    
    let loadingForm= document.querySelector('.loadingEmail');
    let msjEnviado=document.querySelector('.msjEnviado');
     ...
     setTimeout(() => {
       $.ajax({
        ...
        success:function(data){
          loadingForm.classList.add('ocultar');
          msjEnviado.classList.remove('ocultar')
            setTimeout(() => {
              msjEnviado.classList.add('ocultar')
            }, 2500);
        } 
      }); 
     }, 1500);

      loadingForm.classList.remove('ocultar'); 
  });
})(jQuery);
